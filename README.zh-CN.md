...
- **检测 → 续写 → 折叠** — 识别 `518n−2` 指纹，重放该轮 reasoning 并附续写提示，将全部轮次
  折叠为单个响应。
- **窄范围零推理重试** — 对 high-effort gpt-5.5 且终止时
  `reasoning_tokens == 0` 的轮次，复用同一套有上限的折叠路径重试
  （[详情](docs/zero-reasoning-retry.md)）。
...
1. **检测** — `reasoning_tokens == 518n − 2`（`1 ≤ n ≤ 6`，续写上限 3 轮）即判定该轮被截断。
   另有一条窄范围规则，会重试 high-effort gpt-5.5 且终止时零 reasoning token 的轮次
   （[详情](docs/zero-reasoning-retry.md)）。
...
**会影响正常（未截断）的轮次吗？**
不会。干净轮次逐字节透传；折叠路径只在检出 `518n−2` 截断，或命中窄范围 high-effort
gpt-5.5 零推理规则时介入。
